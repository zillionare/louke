...
- `### Spec 链接`: `https://github.com/zillionare/specforge/blob/releases/v0.4/specs/v0.4-004-quote-dialogue/spec.md#fr-XXX`
- `### 验收标准`: `AC-1: ...`, `AC-2: ...`, ... 连续编号

## verify_issue_schema.py 验证结果 (Step 6 跑)

```
$ python3 tools/verify_issue_schema.py --spec v0.4-004-quote-dialogue --branch releases/v0.4
```

**spec 004 视角**: 8/8 issue 通过 (#53, #56, #57, #58, #59, #60, #61, #62)

**全 Feature issue**: 38 个, 18 通过, 20 失败. 失败 20 个全部属于旧 spec 001/003 时代的 issue, 不属于 spec 004 范围.

修复内容: tools/verify_issue_schema.py 扩展支持 NFR-XXX (FR-only regex was incomplete).
