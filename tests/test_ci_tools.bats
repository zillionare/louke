...
@test "CI_TOOLS_T08_ci_scan_pass: aggregates both tools" {
    run bash "$REPO_ROOT/bin/specforge" ci-scan --acceptance "$FIX/acceptance.md" --tests "$FIX/tests_good"
    [ "$status" -eq 0 ]
    [[ "$output" =~ "check_acs" ]]
    [[ "$output" =~ "check_assertions" ]]
}

@test "CI_TOOLS_T09_ci_scan_json: emits merged JSON" {
    run bash "$REPO_ROOT/bin/specforge" ci-scan --acceptance "$FIX/acceptance.md" --tests "$FIX/tests_good" --json
    [ "$status" -eq 0 ]
    echo "$output" | python3 -c "import json,sys; d=json.load(sys.stdin); assert d['ok'] is True; assert 'acs' in d and 'assertions' in d"
}

@test "CI_TOOLS_T10_test_plan_template: strategy doc, no coverage matrix table" {
    TEMPLATE="$REPO_ROOT/templates/test-plan.md"
    grep -q "测试策略" "$TEMPLATE"
    grep -q "AC 追溯约定" "$TEMPLATE"
    grep -q "specforge ci-scan" "$TEMPLATE"
