...
# (2) define AC-FRXXX-YY traceability convention, (3) define lk archer ci-scan CI gate.
...
# Archer only reads issue list and closes the loop via `lk archer ci-scan` schema reverse lookup; explicit reference to verify_issue_schema
...
@test "ARCHER-FORM-004: archer_documents_CI_gate_command" {
    # CI gate uses lk archer ci-scan (replaces old specforge ci-scan)
    run grep -qE "(lk archer ci-scan|ci-scan)" "$ARCHER"
    [ "$status" -eq 0 ] || {
        echo "FAIL: Archer.md does not define lk archer ci-scan CI gate"
        false
    }
}
