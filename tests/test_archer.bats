...
# (2) 定义 AC-FRXXX-YY 追溯约定, (3) 定义 hp archer ci-scan CI 门禁。
...
@test "ARCHER-FORM-004: archer_documents_CI_gate_command" {
    # CI 门禁用 hp archer ci-scan (取代旧的 specforge ci-scan)
    run grep -qE "(hp archer ci-scan|ci-scan)" "$ARCHER"
    [ "$status" -eq 0 ] || {
        echo "FAIL: Archer.md 未定义 hp archer ci-scan CI 门禁"
        false
    }
}
