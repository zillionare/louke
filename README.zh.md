...
每个 agent 在 OpenCode 内跑一个 primary 模型（带同档 fallback）。档位反映**能力**（推理深度、上下文窗口、tool-use 可靠性），不是成本。当前开源模型集合分 3 档；闭源列展示对应商业模型供参考。

| Agent         | 档 | 开源示例                       | 闭源示例（参考）                       |
| ------------- | :-: | ------------------------------ | -------------------------------------- |
...
分布：**3 个 S 档**（Maestro, Sage, Lex）做高风险综合与审稿；**7 个 A 档**（Warden, Archer, Devon, Prism, Keeper, Shield, Judge）覆盖主体实现 + 验证；**2 个 B 档**（Scout, Librarian）做例行维护与交互。

通过 `~/.louke/models.json` 或 `.louke/models.json` 覆盖（参见 [FR-0201](.louke/project/specs/v0.6-008-louke-v030-usability-closure/spec.md)）；`lk models list` / `lk models doctor` 展示当前绑定，`lk models bind <abstract> <full>` 按 agent 覆盖。

### 安装

> **平台支持**：macOS / Linux 专用。Windows 用户请走 [WSL2](https://learn.microsoft.com/zh-cn/windows/wsl/) 或 Docker。`install.sh` 自带 `uname -s` 检查，在不支持的平台上会清晰报错并退出。
