...
- **10 个 canonical semantic Agent** = 实施者与评审者分离；确定性门禁归 Runtime
...
| 阶段        | 实施者        | 评审者               | 说明                                |
| ----------- | ------------- | -------------------- | ----------------------------------- |
| M-FOUND     | Runtime program | none               | 项目奠基检查                         |
...
| M-DEV       | Devon         | **Prism → Runtime ★** | 代码 + 单元测试                    |
| M-E2E       | Shield        | **Prism → Runtime ★** | e2e 测试                           |
| M-BUGFIX    | Devon         | **Runtime ★**         | Bug 修复                            |
...
### 6.1 Agent 权限矩阵 (v0.14)

canonical semantic Agent 有显式 `permission:` 块。Runtime 负责 foundation、quality、regression
和 workflow state；旧名称只保留为 compatibility CLI，不是 semantic Agent 或 board prompt。
...
| Lex / Devon / Shield / Prism / Librarian | `subagent` | ❌ | `task: deny`, `question: deny` | 非交互式；显式 permission 块 |

> 跑 `lk agent lint` 校验所有 agent frontmatter 合规。

### 6.2 分层编排 (v0.14)

**Maestro 是唯一的 `mode: primary` agent**。语义 subagent 不在 OpenCode `<Leader>a`
列表里；Maestro 只路由意图并汇总证据，Runtime 派发程序并拥有 workflow state。

`lk agent scout|warden|keeper` 仅作为 deprecated compatibility adapter 保留，调用同一
Runtime handler，不创建 semantic Agent session，也不写 stage authority。
