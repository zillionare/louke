...
`louke` 定义了 12 个专业 Agent、10 阶段流水线、一个 `lk` CLI——让每次转换都是真正的检查，不是"agent 互相 review"那种软约束。名字就是机制：**在每个 point 上，工作被 hold（= 验证 + 卡点）**。
...
★ **HOLD POINT**——工具强制检查（`lk` CLI 返回 0/1；不通过就不前进）
...
- **`lk` CLI** = 工具强制的 hold point（返回 0/1）
