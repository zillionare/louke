...
| ------------- | -------------------------------------- | :------: | ------------------- | --------------------------------------- |
| **Maestro**   | 指挥家 — 协调整支流水线                |    S     | `minimax-m3`        | `gpt-5.6`, `fable`                      |
| **Sage**      | 贤者 — 苏格拉底式追问需求              |    S     | `glm-5.2`           | `gpt-5.6`, `fable`                      |
| **Judge**     | 裁判 — S 级深度安全审计                |    S     | `minimax-m3`        | `gpt-5.6`, `fable`                      |
| **Archer**    | 射手/架构师 — 设计 test-plan + 架构    |    S     | `glm-5.2`           | `gpt-5.6`, `fable`                      |
...
M-FOUND 奠基步骤（创建 GitHub repo/Project/smoke issue）在 `lk init` 之后单独运行 `lk scout foundation`。
...
每个 agent 有默认模型；日常开发中把编码 Agent 切到本地小模型即可省大部分 token。
...
## Sage 的需求讨论：quote-block 协议

Sage 在 spec 里用 Markdown quote block 开启讨论：
