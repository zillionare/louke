...
PATTERNS: list[tuple[str, re.Pattern[str], str]] = [
    (
        "FAKE-001",
        re.compile(r"\bassert\s+(True\b|1\b(?!\s*[+\-*/]))"),
        "assert True/1 has no value",
    ),
    (
        "FAKE-002",
        re.compile(r"\bassert\s+.+\s+is\s+not\s+None\b"),
        "weak non-null assertion without AC",
    ),
    (
        "FAKE-003",
        re.compile(r"except\b[^:\n]*:\s*(?:#.*)?$\n\s*pass\b", re.M),
        "try/except/pass swallows exceptions",
    ),
    (
        "FAKE-004",
        re.compile(r"except\s+Exception\b[^:\n]*:\s*(?:#.*)?$\n\s*pass\b", re.M),
        "except Exception/pass swallows exceptions",
    ),
    ...
    (
        ...
        "skip/xfail without issue/AC/URL",
    ),
    (
        "FAKE-008",
        re.compile(r"\b(TODO|NotImplemented)\b"),
        "unfinished test marker without issue/URL",
    ),
]
...
def has_context_exception(
    text: str, start: int, end: int, require_ac: bool = False
) -> bool:
...
def scan_file(path: Path) -> list[dict[str, Any]]:
    ...
    for code, pattern, msg in PATTERNS:
        for m in pattern.finditer(text):
            if code == "FAKE-002" and has_context_exception(
                text, m.start(), m.end(), require_ac=True
            ):
                continue
            if code in {"FAKE-005", "FAKE-006", "FAKE-008"} and has_context_exception(
                text, m.start(), m.end()
            ):
                continue
            line = text.count("\n", 0, m.start()) + 1
            violations.append(
                {"code": code, "file": str(path), "line": line, "message": msg}
            )
