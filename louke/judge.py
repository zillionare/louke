...
from ._common import get_diff_files, print_findings
...
def register(subparsers):
    ...
    p = sub.add_parser(
        "security-audit", help="per-milestone deep security audit (FR-0610 two-stage)"
    )
    p.add_argument(
        "--release", required=True, help="release branch, e.g. releases/v0.1"
    )
    p.add_argument("--baseline", default="main", help="baseline (default main)")
    p.add_argument(
        "--checklist",
        default=".louke/templates/security-checklist.md",
        help="audit baseline (default security-checklist.md)",
    )
    p.add_argument(
        "--use-llm",
        action="store_true",
        help="stage 2: agent semantic review (requires configured model)",
    )
    p.add_argument(
        "--model",
        default="",
        help="override default review model (LOUKE_OPENCODE_REVIEW_MODEL)",
    )
...
def cmd_security_audit(args):
    ...
    """
    cwd = Path.cwd()
    print("=== Security Audit (lk agent judge) ===")
    ...
    if args.use_llm:
        ...
        if not model:
            ...
            report_path = (
                raw_dir / f"security-audit-{datetime.now().strftime('%Y%m%d')}.json"
            )
            report_path.write_text(
                json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
            )
            print(
                "warn: --use-llm but no model configured (set LOUKE_OPENCODE_REVIEW_MODEL)",
                file=sys.stderr,
            )
            print(
                f"      stage 2 skipped; report written: {report_path}", file=sys.stderr
            )
        ...
        print(
            f"Stage 2: model={model} (placeholder; integrate with OpenCode review agent)"
        )
        report["stage2_findings"] = [
            {"note": f"would invoke {model} for semantic review"}
        ]
    ...
    raw_dir.mkdir(parents=True, exist_ok=True)
    report_path = (
        raw_dir / f"security-audit-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
    )
    report_path.write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    ...
    if verdict == "fail":
        print("-> verdict: REJECT (critical/high patterns present)")
        return 1
    if verdict == "needs-human-review":
        print(
            "-> verdict: needs-human-review (medium/low non-blocking; needs S-level Judge review)"
        )
        return 2
    print("-> verdict: PASS (stage 1 has no blocking findings)")
    return 0
