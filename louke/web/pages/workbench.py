...
function renderSidebar(activity){{const sidebar=document.querySelector('[data-louke-region="sidebar"]');sidebar.dataset.sidebarKind=activity;sidebar.querySelectorAll('section[data-sidebar-kind]').forEach(s=>s.hidden=s.dataset.sidebarKind!==activity);}}
 const transcripts={{}}; const sessions={{}}; const renderedMessages={{}}; const chatPollers={{}}; let activeAgent='Maestro';
 ...
 async function waitForChatReply(agent,id){{const deadline=Date.now()+120000;let delay=400;while(Date.now()<deadline){{const items=await refreshChat(agent);if(items.some(message=>message.role==='assistant'&&message.content)){{showToast('');return true;}}await new Promise(resolve=>setTimeout(resolve,delay));delay=Math.min(2000,Math.round(delay*1.35));}}showToast('Agent 仍在运行，可稍后继续查看 transcript');return false;}}
 async function sendChat(content){{const agent=activeAgent;const input=document.querySelector('[data-testid="chat-input"]');const button=document.querySelector('[data-testid="chat-submit"]');button.disabled=true;showToast('');try{{const id=await chatSession(agent);const response=await fetch('/api/opencode/instances/'+encodeURIComponent(id)+'/messages',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{content}})}});const payload=await response.json();if(!response.ok)throw new Error(payload.message||payload.error||'Chat request failed ('+response.status+')');addChatMessage(agent,payload.message);input.value='';showToast('Agent 正在回复…');await waitForChatReply(agent,id);}}catch(error){{showToast(error.message||'Chat request failed');}}finally{{button.disabled=false;input.focus();}}}}
