...
def register(subparsers):
    parser = subparsers.add_parser(
        "archer", help="test-plan + architecture design (Archer)"
    )
    ...
    p = sub.add_parser(
        "commit-design",
        help="commit test-plan + architecture + interfaces (git add + commit + push)",
    )
    p.add_argument("--spec", required=True)
    ...
    p = sub.add_parser(
        "validate-test-plan",
        help="validate test-plan.md structure (M-TESTPLAN holdpoint)",
    )
    p.add_argument("--spec", required=True)

    # validate-arch: FR-0700 M-ARCH holdpoint (structure check)
    p = sub.add_parser(
        "validate-arch", help="validate architecture.md structure (M-ARCH holdpoint)"
    )
    p.add_argument("--spec", required=True)
...
def cmd_check_acs(args):
    ...
    result = subprocess.run(
        [
            ...
            "--acceptance",
            f".louke/project/specs/{args.spec}/acceptance.md",
        ],
        cwd=Path.cwd(),
    )
    return result.returncode
...
def cmd_commit_design(args):
    ...
    cmds = [
        [
            "git",
            "add",
            f"{spec_path}/test-plan.md",
            f"{spec_path}/architecture.md",
            f"{spec_path}/interfaces.md",
        ],
...
def cmd_validate_test_plan(args):
    ...
    if "## 1. 立场与边界" not in text and "## 测试策略" not in text:
        failures.append(
            "missing test strategy section (## 1. 立场与边界 or ## 测试策略)"
        )
