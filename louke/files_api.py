...
async def list_files(request: Request):
    ...
    try:
        if view == "content":
            if not rel:
                return JSONResponse({"error_code": "VALIDATION_ERROR", "message": "path required"},
                                    status_code=400)
            from .security import WorkspaceSecurity
            ...
            if line_count > 500 and not approved:
                return JSONResponse({"error_code": "APPROVAL_REQUIRED",
        ...
        if view in ("tree", "changes", "documents"):
            ...
            if view == "changes":
                ...
                for e in entries:
                    e["changed"] = e["path"] in changed or e["path"].lstrip("./") in changed
            return JSONResponse({"view": view, "entries": entries})
        return JSONResponse({"error_code": "VALIDATION_ERROR",
                             "message": f"unknown view {view!r}"}, status_code=400)
