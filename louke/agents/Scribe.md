...
- 当存在面向人的交互入口时，记录关键用户动作、所在界面上下文、用户可见结果和适用的失败/恢复反馈，为后续交互合同提供 Story 事实。
...
- **交互闭环**：若用户通过 Web、Chat、CLI、mobile 等入口操作，每个关键动作发生在哪个 surface/context，用户能看到什么、还能做什么；进行中、成功、失败、只读、dirty、stale、冲突或权限不足等状态中哪些适用，用户如何继续、重试、取消或恢复？Story 只需记录产品事实和关键边界，不设计组件树、CSS 或内部 API。
- **继承边界**：若声称“复用现有界面/交互”，应记录可核验的既有 artifact identity、revision/digest 或需求锚点，并说明哪些行为保持不变、哪些由本 Story 新增或改变；无法核验时标为 `[待补充]`，不得凭名称推断。
...
- **交互事实**：若存在面向人的入口，关键动作的 surface/context、可见结果、适用状态与恢复是否足以让 Sage 转换为合同；复用既有交互时是否给出了可核验来源？
