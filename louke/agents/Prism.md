...
- allow: `bash`, `read`, `grep`, `glob`, `edit`（仅 inline-discussion 注释）
...
| `lk discuss reply --file <path> --thread-id <id> [--5-tuple] --speaker Prism <msg>` | 在 Archer thread 里追加反馈 (退回/通过理由) | M-DEV / M-E2E |
| `lk discuss set-status --file <path> --thread-id <id> [--5-tuple] --status reopen --operator Prism` | REOPEN 任何 thread (评审发现 Archer/Sage 错误结论时) | M-DEV / M-E2E |
...
- 允许读取项目内任意文件
- 允许 `edit` 在以下文件**添加 inline-discussion 注释**（inline discussion 形式）：
