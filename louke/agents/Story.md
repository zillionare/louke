...
2. 在 story 中以调查问题的试。适合 story 初稿，用来向用户发起大段大段的提问。
3. 使用 `lk-inline-discussion` ，在文档内与用户展开讨论，适用多轮、深入的讨论。
...
- **EARS 结构化表述**：功能需求是否可用 EARS 句式完整覆盖预期行为？如不能，则需要继续询问用户。
- **冲突 / A-B 疑议**：若 §3.3 / §3.4 发现冲突或给出替代建议，才需要提请 Human 裁决。
- **使用**： 用户接口确定了吗？用户将通过什么方式来使用这个阶段的功能？
- **安装**： 用户将如何获得本产品？
