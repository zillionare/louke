...
- **目录、包、模块、文件名必须是语义化的、跨版本稳定的**——命名表达**意图（what）**，不表达**实现细节、版本号或时间（how / when）**。版本信息放在 `__version__` / 构建元数据中，而非标识符里。
  - ❌ 默认禁止在标识符中嵌入版本号或时间/状态前缀：`cli_v12.py`、`api_v2/`、`new_calculator.py`、`legacy_helpers/`、`utils_v2/`。
  - ✅ 默认用稳定的名词或业务概念：`cli.py`、`api.py`、`order_service.py`、`pricing/`。
  - **可允许的例外：版本共存窗口**——当 spec（spec.md 或 architecture.md）明确声明了共存窗口、共存原因、退出条件时，**只在新增的那一侧**加版本号（旧侧保持原名），且共存期结束必须立即合并或下架。
  - **重命名是重构，不是新模块**：除共存窗口例外外，语义变了应原地重命名（必要时 `aliases.py` 做 shim），而不是新增 `xxx_v2`——新增 `v2` 等于承认"上一个版本没人迁移"，是设计失败。
...
❌ 在目录/包/模块/文件名中嵌入版本号或时间/状态前缀（`cli_v12.py`、`api_v2/`、`new_xxx.py`、`legacy_xxx.py` 等）——默认禁止；唯一例外是 spec 明确声明的新旧版本共存窗口，且只在"新"的一侧加版本号；否则语义变了应原地重命名而非新建 `v2`
