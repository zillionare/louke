...
def register(subparsers):
    ...
    p = sub.add_parser(
        "commit-rgr", help="commit per R-G-R standard (FR-0580 default no-push)"
    )
    p.add_argument(
        "--phase",
        required=True,
        help="R-G-R phase (green/refactor); red phase deprecated",
    )
    p.add_argument("--message", required=True, help="commit message body")
    p.add_argument(
        "--issue", required=True, help="GitHub issue number currently being handled"
    )
    p.add_argument(
        "--label",
        default="",
        help="force issue type (feature/fix); default reads issue labels",
    )
    p.add_argument(
        "--push", action="store_true", help="explicit push (default no-push)"
    )
...
def cmd_commit_rgr(args):
    ...
    if args.phase == "red":
        print(
            "error: --phase red deprecated (v0.7-001); Red phase no longer commits",
            file=sys.stderr,
        )
        return 1
    if args.phase not in ("green", "refactor"):
        print(
            f"error: --phase must be 'green' or 'refactor', got {args.phase!r}",
            file=sys.stderr,
        )
        return 1
    ...
    if label not in ("feature", "fix"):
        print(
            f"warning: unknown label {label!r}, falling back to 'feature'",
            file=sys.stderr,
        )
        label = "feature"
