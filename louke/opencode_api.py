...
async def create_instance(request: Request):
    inst = get_default_adapter().create(
...
async def list_instances(request: Request):
    return JSONResponse(
        {"instances": [i.to_dict() for i in get_default_adapter().list()]}
    )
...
async def send_message(request: Request):
        ...
        return JSONResponse(
            {
                ...
                "message": f"unknown instance {inst_id}",
            },
            status_code=404,
        )
    ...
    try:
        ...
        return JSONResponse(
            {"error_code": "INSTANCE_NOT_RUNNING", "message": str(e)}, status_code=409
        )
    return JSONResponse(
        {"message": msg.to_dict(), "accepted": accepted}, status_code=202
    )
...
async def list_messages(request: Request):
    ...
    return JSONResponse(
        {"instance_id": inst_id, "messages": [m.to_dict() for m in msgs]}
    )
