...
from ._common import package_root, ensure_gitignore_line
...
def _add_arguments(parser):
    ...
    parser.add_argument(
        "--no-cron",
        action="store_true",
        help="skip installing the daily wiki distillation cron",
    )
    parser.add_argument(
        "--cron-time",
        default="0 4",
        help='cron trigger time (default "0 4" = daily at 04:00)',
    )
...
def _write_default_agent(root: Path, args, report: dict[str, list[str]]) -> int:
    ...
    if existing and existing != "maestro" and not args.force_default_agent:
        print(
            "default_agent already set; use --force-default-agent to overwrite",
            file=sys.stderr,
        )
        return 1
    if existing == "maestro":
...
def _install_cron(root: Path, cron_time: str, report: dict[str, list[str]]) -> int:
    ...
    if sys.platform == "win32":
        print(
            "cron install is not supported on Windows (use WSL or a manual scheduled task)",
            file=sys.stderr,
        )
        return 0
...
def cmd_init(args):
    ...
    if not args.no_issue_template:
        _write_file_if_needed(
            pkg / ".github/ISSUE_TEMPLATE/feature.yml",
            target / ".github/ISSUE_TEMPLATE/feature.yml",
            args,
            report,
        )
        _write_file_if_needed(
            pkg / ".github/ISSUE_TEMPLATE/bug.yml",
    ...
    if not args.no_gitignore:
        for line in (".louke/templates/",):
            ensure_gitignore_line(
                target / ".gitignore", line, dry_run=args.dry_run, report=report
            )
    ...
    if args.board == "opencode":
        ...
        board_args = argparse.Namespace(
            command="opencode", dry_run=args.dry_run, root=target, quiet=args.json
        )
