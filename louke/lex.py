...
def register(subparsers):
    ...
    p.add_argument('--repo', required=True, help='owner/repo (被开发的项目, 不是 louke 框架本身)')
...
def cmd_verify_acceptance(args):
    """调用 louke._tools.verify_acceptance."""
    result = subprocess.run(
        [sys.executable, '-m', 'louke._tools.verify_acceptance',
...
def cmd_verify_issue(args):
    """调用 louke._tools.verify_issue_schema."""
    result = subprocess.run(
        [sys.executable, '-m', 'louke._tools.verify_issue_schema',
         '--spec', args.spec, '--repo', args.repo],
        cwd=Path.cwd(),
    )
    return result.returncode
...
def cmd_quote_check(args):
    """调用 louke._tools.quote_parser (同 Sage quote-check)."""
