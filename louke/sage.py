...
def register(subparsers):
    ...
    p = sub.add_parser(
        "review-testplan",
        help="review test-plan consistency and persist stage artifact",
    )
    p.add_argument("--spec", required=True)
    p.add_argument(
        "--reviewed-target",
    ...
    p = sub.add_parser(
        "record-testplan-review",
        help="persist Sage test-plan review verdict as a stage artifact",
    )
    p.add_argument("--spec", required=True)
...
def _write_testplan_review_artifact(
    *,
    spec_id,
    verdict,
    reviewed_targets,
...
def _review_testplan_documents(spec_id):
    ...
    if (
        "AC-" not in contents["acceptance.md"]
        and "ac-fr-" not in contents["acceptance.md"].lower()
    ):
        failures.append(
            "acceptance.md must define acceptance criteria before test-plan review"
        )
