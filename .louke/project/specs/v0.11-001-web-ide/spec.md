...
我希望把现在的、完全由 maetron推动的工作流，改为由**程序固定的工作流**--但在中间会咨询 maestro，实现**部分情况下的动态工作流**，或者在**多个工作流之
间选择**。比如，现在M-FOUNDATION 是由 scout 和 warden 来完成的。但是，如果改用程序执行，就没必要有这两个 Agent，而且 M-FOUNDATION也不必成为 maestro 工作流中的一部分。又比如， Maestro 常常跳过 M-LOCK，通过程序，我们可以确保这一步得到人类的确认。

此外，我也需要理解一些技术背景。我的问题是：

1. 通过web 创建 opencode 运行实例之后，如何 detach, attach和退出？
2. 新创建的 Opencode实例，进行会话时，它的上下文来自何处？
3. 我们可以定制 opencode 上下文实例吗？如果可以，在每个具体的步骤--比如调用 devon 来实现某一批 github issue 时，应该如何构建上下文？
4. 如何构建由程序和 agent 组成的动态工作流？

总之，这是一个探索性任务。你可以先扫描 louke 这个项目，结合我前面的给出的背景，对如何做一个更好的 louke (beyond vibes, into craft)给出建议
