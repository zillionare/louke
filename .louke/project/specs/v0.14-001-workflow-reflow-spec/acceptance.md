---
status: Draft v9 (M-ACC revision after Lex REJECT round 2; pending review)
...
spec_digest: sha256:32b2f4c51209b0c8e4167439533370877ad38040fb44ae696d20d01280c81069
acceptance_digest: __ACCEPTANCE_DIGEST__
doc_index_digest_sha256: __DOC_INDEX_DIGEST__
revision: 9
lex_review_artifact: .louke/project/specs/v0.14-001-workflow-reflow-spec/acceptance-review.md
author: Sage
---

# v0.14 Workflow Reflow — Acceptance Criteria

> 每节与 `spec.md` 的一个 FR/NFR 一一对应。所有条件只能通过公开Web/API、持久化artifact/event或外部Git/GitHub/OpenCode边界断言。`flow.md`中的旧过程名`M-REQ-APPROVAL`与本合同canonical名称`M-LOCK-1`是同一个节点。M-LOCK-1批准后本acceptance与spec共同digest进入只读；FR-1800 Issue创建以此digest为reconcile identity之一。
...
- **Then** 两次均可访问Workbench release入口，setup manifest中的workspace/repository、provider namespace与readiness identity保持相同，两次启动均不产生workspace级配置修改或具体release级资源创建调用。

### AC-FR0100-03
- **Given** Python/runtime/package/端口或服务本体无法加载，使Web服务进程无法建立
- **When** `lk serve`结束
- **Then** 进程退出码非0，stderr含该硬前置失败项与非空修复动作，Web监听地址不可访问，且WorkflowRun与外部create调用数均为0；同一失败项不会同时出现在可访问Web的`BLOCKED` readiness诊断中。

<a id="ac-fr-0200"></a>
# FR-0200 Workspace Setup Preview、确认与 Manifest

### AC-FR0200-01
- **Given** Git remote、项目资料和认证身份产生workspace/repository identity、owner/provider namespace、认证、模型、OpenCode及Backlog/release-project namespace或创建能力候选值
- **When** Human打开Setup preview
- **Then** 每个必要workspace级字段显示value与provenance，且确认前preview所列workspace级外部配置未被创建或修改；无论Setup是否随后完成，在release请求确认前，具体release的Project、WorkflowRun、release GitHub Project、branch及Spec目录数量变化均为0。

### AC-FR0200-02
- **Given** 两个权威来源对owner产生冲突
- **When** Human未裁决并重启`lk serve`
- **Then** setup仍为`waiting_human`，两个候选及provenance保持字节相等，WorkflowRun数和外部配置相对操作前均无变化。

### AC-FR0200-03
- **Given** Human对setup revision R选择候选值并确认
- **When** Runtime完成或重试setup
- **Then** setup manifest记录R、actor、选择、全部候选来源、workspace/repository与provider namespace非秘密身份、auth/model/OpenCode readiness、namespace/create capability及operation evidence；相同确认重试不重复修改workspace级配置，且具体release的Project、WorkflowRun、release GitHub Project、branch和Spec目录数量及identity与确认前相同。

### AC-FR0200-04
- **Given** workspace级release-project namespace或创建能力的可读检查返回缺失、多个namespace、身份冲突或权限不足
- **When** setup reconcile执行
- **Then** Web显示`waiting_human`及精确检查详情，不按模糊名称选择namespace，setup manifest不标记complete，且不通过创建具体release Project来探测能力。

### AC-FR0200-05
- **Given** Setup确认后部分workspace级配置已更新，而后续可读检查因断网、权限失败或服务重启中断
- **When** Human从同一workspace再次执行`lk serve`并打开Setup
- **Then** 页面恢复同一setup revision，逐项显示已完成、失败或不确定的workspace级配置/检查及修复方向；重试先核对已完成操作，仅继续未完成项，全部必要workspace级事实一致前仍留在Setup，且具体release级资源的创建调用数为0、既有数量与identity不变。

<a id="ac-fr-0300"></a>
# FR-0300 Web Release 请求与单活跃主 Release

### AC-FR0300-01
- **Given** `/projects/new`收到尚未确认的preview请求，其中story/version可以有效，也可以为空或非法
- **When** Human请求preview但未执行确认
- **Then** 具体release的Project、WorkflowRun、release GitHub Project、release branch和Spec目录数量均不变；输入为空或非法时页面另显示字段级错误，且Backlog entry数量也不变。
...
- **Then** Web显示相关full ref与SHA或refresh错误，状态非PASS，本次release的Project、WorkflowRun、release GitHub Project、release branch、Spec目录和M-STORY task均未创建。
...
### AC-FR0400-04
- **Given** 本次release尚无任何release级资源，declared remote刷新成功，但本地`main`与权威remote `main`不一致，或上一开发分支相对权威`main`为ahead、behind、diverged或无法判定
- **When** Runtime执行foundation前置检查
- **Then** Web显示每个相关full ref、SHA、关系与非空remediation，foundation状态非PASS；本次release的Project、WorkflowRun、release GitHub Project、release branch、Spec目录及M-STORY task数量均不增加，且Human完成修复并重新检查通过前，确认请求不能绕过阻塞。

### AC-FR0400-05
- **Given** Runtime恢复同一foundation时，发现已存在的release branch起点不等于本次检查所得权威`main` SHA M，或已存在的release branch或其它release资源与Foundation evidence要求的stable identity冲突
- **When** Runtime reconcile已发生的release副作用
- **Then** Foundation保持`conflict`或`needs_attention`且不标记complete，不进入M-STORY，不创建其它候选release branch或release资源，也不静默改写已存在资源；Web显示冲突资源、实际与预期identity及非空remediation，Human修复冲突并完成精确identity reconcile前M-STORY task数量不增加。
...
### AC-FR0600-03
- **Given** 页面携带旧revision的保存、review或推进请求，或请求当前阶段不允许的动作
- **When** Runtime校验该请求
- **Then** run、artifact和外部资源均不改变，页面显示冲突所针对的当前revision、当前步骤、待处理Human动作及可继续位置；Human可从该位置继续而无需另行执行CLI流程命令。
...
- **When** 两者保存同一文档
- **Then** 最多一个成功，失败方看见当前revision/version和冲突原因，落盘bytes等于成功者内容且没有静默覆盖。
...
- **Then** `no comment`不可用，绕过页面直接提交也被拒绝，页面仍显示本轮已编辑且Human verdict不为PASS。
...
- **Then** 两者均绑定R；Lex未取得写权时不能改变文档或discussion，取得写权后留下的thread可从当前review页面和持久化discussion出口查询。
...
- **Then** M-SPEC只显示M-STORY，M-ACC显示M-SPEC及M-STORY；提交任意其它target被拒绝并显示合法目标，revision与流程步骤不变。
...
### AC-FR1600-04
- **Given** 当前Acceptance revision进入Human/Lex review
- **When** Human提交编辑或`comment`、Lex给出非PASS、存在未解决discussion，或格式验收失败
- **Then** 页面显示各自绑定的revision/verdict或格式错误，原Sage上下文收到完整意见并形成下一revision；只有Human `no comment`、Lex PASS、零未解决discussion、格式PASS且Story/Spec digests仍匹配时才离开M-ACC。
...
- **When** Human进入Project current页
- **Then** 页面可见当前步骤为等待批准的`M-LOCK-1`，显示S、P、A所标识的三份版本并提供可用批准动作；Agent、旧challenge、错误revision或错误joint digest批准均被拒绝且gate仍pending。

### AC-FR1700-03
- **Given** 已认证Human批准当前challenge/joint digest
- **When** 再通过Web/API写三份文档之一
- **Then** 批准记录含actor/time/challenge/revision/digests，页面显示三份文档已锁定且后续写请求被拒绝，三文件bytes保持不变；Project current页继续显示Issue reconcile进度或其可继续位置。
...
### AC-FR1800-02
- **Given** 锁定Spec为本合同当前revision，含21个FR和3个NFR且24项全部Valid=`✅`，并各有对应Acceptance section
- **When** Issue operation全部成功
- **Then** 按该锁定revision的当前有效单元数计算目标，每个Valid不为`❌`的requirement恰有一个Issue；在本场景中目标数为24，其title以精确单一`[{ID}]`token开头，body包含同一ID、锁定Spec section URL与Acceptance section URL，且24个Issue均作为单一Project item出现在Foundation manifest指定的release GitHub Project中。
...
- **Then** 该Issue不被复用，页面显示冲突状态及不匹配字段；在Human消除歧义前不创建第二个候选Issue。
...
- **When** 两者完成
- **Then** 恰好一个成功，另一个得到当前revision/token及稳定冲突结果，成功副作用只发生一次。
...
- **Then** Web/API显示冲突或`needs_attention`，列出resource kind、provider namespace及冲突字段，且外部资源数量和已记录复用身份均不改变；只有精确字段匹配的单一候选可被复用。
