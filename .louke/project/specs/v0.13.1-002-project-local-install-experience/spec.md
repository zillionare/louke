...
- 当 `<CWD>/.venv/` 已存在时，重新运行 `lk install` 应以非零退出并提示 `lk upgrade`——它 SHALL NOT 静默升级，也 SHALL NOT 修改、删除或重建已有 `.venv/`。
- `lk upgrade` 在浅层意义上是幂等的: 连续运行两次且无中间变化应两次均以 0 退出。

> **Sage** [RESOLVED]: 形式整理发现此处仍引用‘用户确认的拒绝规则’，但本 spec/acceptance 未定义用户确认的具体触发条件、交互或拒绝行为。请确认是否仅保留现有 AC-2 的可观察断言（非零退出、建议 lk upgrade、已有 .venv 不变），还是需要在现有措辞范围内补充规则；Sage 不自行决定。


>> **Maestro**: 按最小语义处理：仅保留现有 AC-2 的可观察断言——已有 .venv 时 lk install 非零退出、提示 lk upgrade、已有环境不被修改；本 spec 不新增用户确认交互或拒绝规则。
...
本版本发布后，测试代码中的 AC 引用编号将从之前的 `{AC代码}` 升级为带版本号的编号，即 `{AC代码}@{version}`。

比如，本版本的版本号为 v0.13.1，AC 为 `AC-NFR1505-01`，则 Devon/Shield 在写测试代码时，需要通过 `AC-NFR1505-01@v0.13.1` 来引用。

兼容策略：

- 兼容旧格式：AC-FR-1510-01（不带 @）仍被解析，但该测试函数不在本版本检查范围内。
- 新格式：AC-FR1510-01@v0.13.1 严格归属当前 spec，用于 trace PASS。
...
通过 pre-commit hook 来执行以下任务：

1. commit message format：检查 commit subject 是否以 `feat: green` / `fix: green` / `refactor:` 等 prefix 开头；
2. AC trace：扫描测试代码第一行 docstring中，是否有AC 编码（见 NFR-1505），该AC码是否在 acceptance.md 中；如果 subject 是以 fix: 开头，则不执行此项。
3. anti-pattern 扫描：抓 FAKE-001/002/003/004/005/006/008 等反模式。

> **Aaron:** 不再强制 R-G-R。这种方式仍然是 Devon 的开发要求，但因为 RED 阶段不许提交；Refactor 阶段又可能没有，所以形同虚设。
> **Aaron:** 本功能实现时，需要同步修改 Maestro 流程，Readme ，以及 Devon
---
...
3. **已存在 venv 时 `lk install`（US-1503）**: 非零退出、提示 `lk upgrade`，且不修改已有 `.venv/`。记录在 NFR-1503 AC-2 中。✅。
