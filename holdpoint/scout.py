...
def cmd_identity_check(args):
    """调用 holdpoint._tools.check_identity."""
    result = subprocess.run(
        [sys.executable, '-m', 'holdpoint._tools.check_identity', '--repo', args.repo],
        cwd=Path.cwd(),
    )
...
def cmd_commit_foundation(args):
    ...
    执行顺序:
    1. git add .holdpoint/project/specs/{spec-id}/*.md
    2. git add .holdpoint/project/project-info.md
    3. git commit -m {message}
    4. git push -u origin releases/{version}

    失败时: 任一命令 exit 非 0 → 中止后续，返回非 0 退出码。
    """
    spec_path = f".holdpoint/project/specs/{args.spec_id}"
    cmds = [
        ['git', 'add', f"{spec_path}/*.md",
         '.holdpoint/project/project-info.md'],
