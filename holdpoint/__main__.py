...
Usage:
    hp <agent> <command> [options]

Examples:
    hp scout identity-check --repo owner/repo
    hp sage quote-check --spec v0.1-001-init
    hp warden foundation-check --repo owner/repo --version v0.1 --spec-id v0.1-001-init
    hp lex verify-acceptance --spec v0.1-001-init
    hp archer ci-scan --spec v0.1-001-init

设计:
- 每 agent 一个模块 (holdpoint/{agent}.py)，暴露 register(subparsers) 和 run(args)
- __main__ 解析 hp <agent> <command>，dispatch 到对应 agent.run()
"""
import argparse
import sys
