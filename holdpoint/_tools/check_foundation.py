...
检查项(F1-F11):
  ...
  F6 project-info 完整:  .holdpoint/project/project-info.md 包含必须字段
                          Version, Repo, Project, Spec ID, Release Branch
  F7 story.md 存在:      .holdpoint/project/specs/{spec-id}/story.md 存在
...
def check_f6_project_info(spec_id: str | None) -> CheckResult:
    ...
    pi_path = Path(".holdpoint/project/project-info.md")
    if not pi_path.is_file():
        r.error = ".holdpoint/project/project-info.md 不存在"
        return r
...
def check_f7_story(spec_id: str) -> CheckResult:
    ...
    story_path = Path(f".holdpoint/project/specs/{spec_id}/story.md")
    if not story_path.is_file():
        r.error = f".holdpoint/project/specs/{spec_id}/story.md 不存在"
        return r

    size = story_path.stat().st_size
    r.passed = True
    r.message = f".holdpoint/project/specs/{spec_id}/story.md ({size} bytes)"
    return r
