...
检查项(L1-L5):
  L1 文件存在:        .holdpoint/project/specs/{id}/acceptance.md 存在
...
使用:
  python tools/verify_acceptance.py --spec v0.1-001-holdpoint
  python tools/verify_acceptance.py --offline \\
      --spec-file .holdpoint/project/specs/v0.1-001-holdpoint/spec.md \\
      --acceptance-file .holdpoint/project/specs/v0.1-001-holdpoint/acceptance.md
"""
...
def fetch_spec_text(spec_id: str) -> str | None:
    """读 main 分支上的 .holdpoint/project/specs/{spec_id}/spec.md"""
    return gh_api_read(f".holdpoint/project/specs/{spec_id}/spec.md")


def fetch_acceptance_text(spec_id: str) -> str | None:
    """读 main 分支上的 .holdpoint/project/specs/{spec_id}/acceptance.md"""
    return gh_api_read(f".holdpoint/project/specs/{spec_id}/acceptance.md")
...
def main() -> int:
    ...
    if args.offline:
        ...
        if spec_text is None:
            print(f"无法读取 main 分支上的 .holdpoint/project/specs/{args.spec}/spec.md", file=sys.stderr)
            return 1
        if acceptance_text is None:
            # 不报错, 走 L1 检查
            pass
