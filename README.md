...
1. 用户把一句话 PRD 写入 `.specforge/project/specs/{spec-id}/story.md` (或直接 chat 告诉 Sage)
...
7. 说明测试环境搭建要求
8. 输出测试计划文档，写入 `.specforge/project/specs/{spec-id}/test-plan.md`
...
**交接产出**：`.specforge/project/specs/{spec-id}/test-plan.md`（含可追溯矩阵），供 Archer 使用。
...
3. 任务拆解 → 每个任务必须包含：
   ...
   - 分支名、描述、依赖关系、是否可并行
4. 排序 → 确定执行顺序，标注可并行任务
5. 输出任务列表文档，写入 `.specforge/project/specs/{spec-id}/task-plan.md`
...
**交接产出**：`.specforge/project/specs/{spec-id}/task-plan.md`（含任务列表 + 依赖图），供 Forge 使用。
...
3. 生成验收报告，写入 `.specforge/project/specs/{spec-id}/acceptance.md`
