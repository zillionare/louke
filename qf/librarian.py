...
def register(subparsers):
    ...
    p = sub.add_parser('distill', help='raw → wiki 蒸馏 (按 LLM, qf 仅准备)')
    p.add_argument('--source', default='.quanti-forge/raw/',
                   help='raw 路径 (默认 .quanti-forge/raw/)')
    p.add_argument('--target', default='.quanti-forge/wiki/pages/',
                   help='wiki 路径 (默认 .quanti-forge/wiki/pages/)')

    p = sub.add_parser('lint', help='wiki 健康检查 (broken links, orphaned pages)')
    p.add_argument('--wiki', default='.quanti-forge/wiki/')
...
def cmd_distill(args):
    """Distill raw → wiki — LLM 蒸馏的 qf 包装.

    LLM 蒸馏是 LLM 的事, qf 仅:
    1. 扫描 raw/ 下 status=resolved 的会话
    2. 生成待蒸馏清单 (供 LLM 阅读)
    3. LLM 蒸馏后, qf 写入 wiki/
    ...
    print(f"\n下一步: LLM 阅读这些条目, 蒸馏后用 qf librarian write <wiki-page> 写入")
    return 0
...
def cmd_from_raw(args):
    ...
    LLM 蒸馏由后续流程调用 qf librarian distill.
    """
    cwd = Path.cwd()
    raw_dir = cwd / '.quanti-forge/raw/'
    wiki_pages_dir = cwd / '.quanti-forge/wiki/pages/'
    ...
    print(f"  下一步: qf librarian distill 调用 LLM 蒸馏这些 raw 草稿")
    return 0