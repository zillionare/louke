...
Sage 对 PRD（竞品分析+实施计划）中的模糊点进行了 3 组苏格拉底追问，用户逐条回答。然后 Sage 产出了 `specs/001-specforge-v0.1/spec.md`，Lex 初次审核发现 3 条阻塞项（FR-007/010/012 验收标准不够具体），退回修正后通过。

## 关键结论

- **V1 范围**: Guide Agent, Librarian Agent, 8 个模板, `specforge init` 工具
- **Librarian 触发**: 手动 + 自动（wiki/pages/ 文件数 ≥ 5）
- **会话保存**: Agent 在返回最终结果前自动写入 wiki/pages/
