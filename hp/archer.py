...
def cmd_ci_scan(args):
    """Wrap tools/ci_scan.py."""
    result = subprocess.run(
        ['python3', 'tools/ci_scan.py',
         '--acceptance', f".quanti-forge/project/specs/{args.spec}/acceptance.md",
         '--tests', 'tests/'],
        cwd=Path.cwd(),
    )
...
def cmd_check_acs(args):
    """Wrap tools/check_acs.py."""
    result = subprocess.run(
        ['python3', 'tools/check_acs.py',
         '--acceptance', f".quanti-forge/project/specs/{args.spec}/acceptance.md"],
        cwd=Path.cwd(),
    )
    return result.returncode
...
def cmd_commit_design(args):
    ...
    spec_path = f".quanti-forge/project/specs/{args.spec}"
    cmds = [
