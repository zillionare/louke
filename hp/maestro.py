...
qf 提供: status 查询 + advance/regress/escalate 操作。
"""
import argparse
import json
import subprocess
...
def cmd_status(args):
    ...
    # 读 project-info.md
    info_path = cwd / '.quanti-forge/project/project-info.md'
    if info_path.exists():
...
def cmd_advance(args):
    ...
    if args.stage == 'M-FOUND':
        ...
        info = cwd / '.quanti-forge/project/project-info.md'
        if info.exists():
        ...
        specs = list((spec_root / '.quanti-forge/project/specs').glob('*/spec.md'))
        if not specs:
            print(f"[medium] 无 spec.md")
        else:
            for sp in specs:
                print(f"  spec: {sp.parent.name}")
                print(f"    [todo: 调 qf sage quote-check 验证]")
    elif args.stage == 'M-DEV':
        # 测试通过 + lint 通过
        print(f"  [todo: 调 qf keeper gate 验证]")
    elif args.stage == 'M-E2E':
        # e2e 通过
        print(f"  [todo: 调 qf shield run-e2e 验证]")
    elif args.stage == 'M-BUGFIX':
        # 回归通过
        print(f"  [todo: 调 qf keeper regression 验证]")
    elif args.stage == 'M-SECURITY':
        # security audit 通过 (or user disabled)
        print(f"  [todo: 调 qf judge security-audit 验证, 或检查 DoD 是否关闭]")
    elif args.stage == 'M-MILESTONE':
        # Librarian 蒸馏完成
        print(f"  [todo: 调 qf librarian from-raw + lint 验证]")
