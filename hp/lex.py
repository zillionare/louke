...
def cmd_verify_acceptance(args):
    """Wrap tools/verify_acceptance.py."""
    result = subprocess.run(
        ['python3', 'tools/verify_acceptance.py', '--spec', args.spec],
        cwd=Path.cwd(),
    )
...
def cmd_verify_issue(args):
    """Wrap tools/verify_issue_schema.py."""
    result = subprocess.run(
        ['python3', 'tools/verify_issue_schema.py', '--spec', args.spec],
        cwd=Path.cwd(),
    )
    return result.returncode
...
def cmd_quote_check(args):
    """Wrap tools/quote_parser.py (同 Sage quote-check)."""
    spec_path = f".quanti-forge/project/specs/{args.spec}/spec.md"
    result = subprocess.run(
        ['python3', 'tools/quote_parser.py', spec_path, '--check-ready'],
        cwd=Path.cwd(),
    )
    return result.returncode