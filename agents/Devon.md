...
1. 创建任务分支：`hp devon branch-create --spec-id {spec-id} --task-id TASK-{N}`
2. 阅读 TEST 计划中该任务关联的测试用例
3. 编写测试代码，精确描述期望行为
4. 运行测试：`hp devon run-tests --scope unit --fast` → 确认失败（Red）
5. 提交测试文件：`hp devon commit-rgr --phase red --task-id TASK-{N} --message "{描述}"`
...
1. 编写刚好使测试通过的实现代码
2. **禁止**添加未由测试驱动的功能
3. 运行测试：`hp devon run-tests --scope unit` → 确认全部通过（Green）
4. 提交实现代码：`hp devon commit-rgr --phase green --task-id TASK-{N} --message "{描述}"`
...
3. **禁止**改变外部行为
4. 提交重构：`hp devon commit-rgr --phase refactor --task-id TASK-{N} --message "{描述}"`
