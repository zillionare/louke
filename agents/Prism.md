...
- **审查测试代码反模式**（8 类，详见 `.louke/templates/test-plan.md` §1.3）—— 防止"测试通过但无意义"
...
- 项目代码结构概览
- `.louke/templates/test-plan.md` §1.3（测试反模式清单，作为 Prism 审查测试代码的基线）
- `.louke/project/specs/{SPEC-ID}/architecture.md` + `interfaces.md`（M-ARCH 评审；如存在）
...
2. **跑完整 review** → `lk prism review --diff HEAD~1..HEAD`（包含 test-patterns + security-quick-scan + code-quality）
3. **生产代码审查** → 可读性、设计模式、DRY、变更影响（人工深读）
4. **测试代码审查** → `lk prism test-patterns --tests tests/`（自动 8 类反模式 + AC 引用检测）
5. **批判性审视** → 质疑设计假设，寻找"看起来 OK 但有暗病"的代码
6. **安全 quick scan** → `lk prism security-quick-scan --diff HEAD~1..HEAD`（浅扫 pattern，深的归 Judge）
...
**基线**: `.louke/templates/security-checklist.md`（参考"输入验证 / 认证 / 数据保护 / 错误处理 / 依赖 / 日志" 章节）。
...
**路径**：`.louke/raw/{yy-mm-dd}/{session-id}.md`，`session-id = {agent}-{spec-id 或 phase}-{议题}`，例 `prism-v0.1-001-code-test-review`
