...
spec 讨论直接在 release 分支（`releases/{version}`）上进行，不另建分支。Lex 读取同一 release 分支上的 `.specforge/project/specs/{spec-id}/spec.md` 和 `.specforge/project/specs/{spec-id}/acceptance.md` 进行审核。
...
- spec 命名必须符合 `.specforge/project/specs/{spec-id}/spec.md` 格式
- 必须能找到对应的 PRD 文档 (`.specforge/project/specs/{spec-id}/{story|prd}.md`)
- **必须存在** `.specforge/project/specs/{spec-id}/acceptance.md` (Sage 负责与 spec.md 同步生成)
...
1. **检查 spec.md 是否 ready** → `specforge quote-check .specforge/project/specs/{id}/spec.md --check-ready`
...
- **必填字段**（form 渲染后的 markdown 形式）：
  - `### 需求 ID` → 内容匹配 `^FR-\d{3}$`
  - `### Spec 链接` → 完整 GitHub URL，fragment 小写 `#fr-XXX` 或 `#nfr-XXX`
  - `### 验收标准` → **v0.5-006 三种形式** (任选其一):
    1. `acceptance.md#ac-fr-XXX` URL (默认, 一个 FR 一个锚, 指向整个 AC 块)
    2. `spec(-vol)?.md#fr-XXX` URL (AC 在 spec 章节中)
...
| L4   | spec 可达性   | `gh api` 拉取 .specforge/project/specs/{id}/spec.md 失败（路径错） |
| L5   | 锚点存在性    | spec.md 中找不到 `#fr-XXX`（FR 被删/重命名）                |
| L6   | 锚点内容      | 锚点上下文无 `FR-XXX` 字样（被错误复用）                    |
| L7   | AC 锚点       | 验收标准字段不是 acceptance.md 完整 URL / fragment 错 / acceptance.md 中找不到 `#ac-fr-XXX` 锚 / 锚点上下文无 `FR-XXX` |
