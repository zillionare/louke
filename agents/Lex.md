...
- **先跑结构化校验** → `qf lex verify-acceptance --spec {spec-id}`（L1 文件存在 / L2 FR-NFR 节对应 / L3 AC 编号连续 / L4 AC 内容非空 / L5 反向覆盖）。任何 L 失败 → 立刻退回 Sage；全过 → 进入语义审核
...
1. **检查 spec.md 是否 ready** → `qf lex quote-check --spec {id}`
...
```bash
qf lex verify-issue --spec {spec-id}
```
...
- [ ] 所有 quote block 都 ✓ resolved (用 `qf lex quote-check --spec {spec-id}` 验证，exit 0)
- [ ] spec 中已含「已知约束与排除项」段
- [ ] `acceptance.md` 与 `spec.md` 同步存在, FR/NFR 编号一一对应
- [ ] 用户已明确确认 spec 锁定
- [ ] `qf lex verify-issue --spec {spec-id}` 返回 `[通过]` (L1-L8 全部通过)
