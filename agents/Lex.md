...
1. **检查 spec.md 是否 ready** → `hp lex quote-check --spec {id}`
...
```bash
hp lex verify-issue --spec {spec-id}
```
...
- [ ] 所有 quote block 都 ✓ resolved (用 `hp lex quote-check --spec {spec-id}` 验证，exit 0)
- [ ] spec 中已含「已知约束与排除项」段
- [ ] `acceptance.md` 与 `spec.md` 同步存在, FR/NFR 编号一一对应
- [ ] 用户已明确确认 spec 锁定
- [ ] `hp lex verify-issue --spec {spec-id}` 返回 `[通过]` (L1-L8 全部通过)
