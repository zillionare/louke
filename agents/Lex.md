...
- 关联 Project (这是 Sage/Lex 协作: 谁创建谁关联, 见阶段二)
...
spec 讨论直接在 release 分支（`releases/{version}`）上进行，不另建分支。Lex 读取同一 release 分支上的 `specs/{spec-id}/spec.md` 进行审核。
...
- 必须能找到对应的 PRD 文档 (`specs/{spec-id}/{story|prd}.md`)
...
- **责任划分**: issue **创建**是 Sage Step 5 职责; Lex **只验证** Sage 是否已创建, 不创建
...
- 若缺失, Lex **不补建**, 而是在 spec.md 追加 quote 通知 Sage 补建: `> **Lex:** FR-XXX 缺对应 issue, 请 Sage 在 Step 5 补建`
...
- **创建** = Sage (Step 5)
- **验证** = Lex (阶段二/三)
- 原因: issue 创建需要 spec 锚点 (`<a id>` 由 Sage Step 4 加), 而 spec 锁定的"操作源"自然由 Sage 维护; Lex 独立验证可避免 "creator 同时是 checker" 的盲点
...
4. **运行 schema 验证器** → 阶段三
5. **关联 Project** → Lex 执行 (这是 Lex 的职责, 因为 PR 权限协作需要 Lex 这个 reviewer 角色)
