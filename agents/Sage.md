...
- 上一阶段产生的 .holdpoint/project/project-info.md（含 **Project ID** 字段）
...
2. 根据 `.holdpoint/templates/spec.md` 模板填充已明确的字段
...
1. **先 commit 用户的 review 改动**（用户经常忘 commit，但**用户的 review 是 Sage 流程的输入**，不 commit 的话 git diff 之后会污染）。命令:
   ```bash
   git add .holdpoint/project/specs/{spec-id}/spec.md
...
SPEC_URL="https://github.com/${REPO}/blob/${BRANCH}/.holdpoint/project/specs/${SPEC_ID}/spec.md"
ACCEPTANCE_URL="https://github.com/${REPO}/blob/${BRANCH}/.holdpoint/project/specs/${SPEC_ID}/acceptance.md"
```
...
PROJECT_URL=$(grep -E '^\- \*\*Project ID\*\*' .holdpoint/project/project-info.md | grep -oE 'https?://[^[:space:]]+')
# 把 issue 加到 Project
...
- `.holdpoint/project/specs/{spec-id}/test-plan.md`（评审对象）
...
命名：`.holdpoint/project/specs/{spec-id}/spec.md`

必须包含（参见 `.holdpoint/templates/spec.md`）：
...
- ✅ `zillionare/quanti-forge@1c02bd2` — GitHub 必定渲染为可点击链接
...
**路径**：`.holdpoint/raw/{yy-mm-dd}/{session-id}.md`，`session-id = {agent}-{spec-id 或 phase}-{议题}`，例 `sage-v0.1-001-clarify-round-2`
