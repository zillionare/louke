...
- 上一阶段产生的 .quanti-forge/project/project-info.md（含 **Project ID** 字段）
...
2. 根据 `.quanti-forge/templates/spec.md` 模板填充已明确的字段
...
SPEC_URL="https://github.com/${REPO}/blob/${BRANCH}/.quanti-forge/project/specs/${SPEC_ID}/spec.md"
ACCEPTANCE_URL="https://github.com/${REPO}/blob/${BRANCH}/.quanti-forge/project/specs/${SPEC_ID}/acceptance.md"
```
...
PROJECT_URL=$(grep -E '^\- \*\*Project ID\*\*' .quanti-forge/project/project-info.md | grep -oE 'https?://[^[:space:]]+')
# 把 issue 加到 Project
...
必须包含（参见 `.quanti-forge/templates/spec.md`）：
...
**路径**：`.quanti-forge/raw/{yy-mm-dd}/{session-id}.md`，`session-id = {agent}-{spec-id 或 phase}-{议题}`，例 `sage-v0.1-001-clarify-round-2`
