...
注意状态字段里`是否已决定`字段的值，一开始都应该设置为 ⚠️。**严禁沉默即同意** — 在用户完成一轮 review 之后，对某个需求，**只有满足以下任一条件**，Sage 才能将 ⚠️ 修改为 ✅:

1. 用户在该 FR/NFR/US 单元的 quote block 中**显式回复**"OK"/"可以"/"确认"/"✅"等肯定语义；
2. 用户在 quote block 中**完整回答了 Sage 关于该 FR 的所有未决问题**且没有新增问题；
3. 用户**显式说**"这一批可以锁定"且该 FR 在此批内（适用于批量确认场景）。

**禁止的等价操作**:
- ❌ 用户**未回复**就因"看起来没否定"标 ✅
- ❌ 用户的回复**未涉及该 FR**（如用户只回了 FR-0001 的问题）就**批量**把 FR-0002/0003/0004 标 ✅
- ❌ 用户**提出新问题**（哪怕只是"这个词是否准确"）就标 ✅ — 必须先回答新问题，等用户再确认
- ❌ 把"用户没时间回"作为标 ✅ 的理由 — 5 轮上限到达时应升级 Maestro 决策（见 Step 3）
...
本步骤是自重复步骤。**最多 5 轮强制结束**（防止在"不能结束 → 标 resolved → 跳过真实歧义"的循环中无限迭代；M3 类小模型在此循环中倾向于快速消掉 ⚠️ 标记以满足退出条件）。轮数 = 用户在 IDE 中完成 review 提交 + Sage 给出响应的完整回合。

你首先要按以下情况，判断本步骤是否可以结束：
...
**轮数硬上限**: 第 5 轮结束时，无论三个条件是否全部满足，Sage **必须**给出 "是否锁定 spec" 的明确二选一提示给用户:
- ✅ "可以锁定" → 用户确认后进入 Step 4
- 🚨 "还有未澄清" → 升级 Maestro 决策（不要 Sage 单方面标 resolved 绕过）

**反模式（特指本步骤）**:
- ❌ 轮数累计 < 5 但 Sage 想"差不多得了"主动把 ⚠️ 改成 ✅
- ❌ 超过 5 轮仍继续追问而不升级
- ❌ 把 ⚠️ 改 ✅ 作为"安抚用户可以锁定"的妥协手段
