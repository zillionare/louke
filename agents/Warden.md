...
```bash
qf warden foundation-check --repo {owner}/{repo} --version {version} --spec-id {Spec-ID} [--upstream main]
```
...
```
读取 `.quanti-forge/project/specs/{Spec-ID}/story.md`
├─ 长度 ≥ 50 字？     （避免空壳 story）
...
1. **运行 `qf warden foundation-check`** → 自动化检查 F1-F11，获取通过/拒绝结果
   - 输出 [拒绝] → 直接输出拒绝原因，不进入人工检查
   - 输出 [通过+警告] → 警告信息（如 F10 orphan、F11 身份漂移提示）需在 Warden 输出中透传
   - 输出 [通过] → 进入步骤 2
2. **读取 `.quanti-forge/project/specs/{Spec-ID}/story.md`** → 按上面三条做内容合理性检查
...
**路径**：`.quanti-forge/raw/{yy-mm-dd}/{session-id}.md`，`session-id = {agent}-{spec-id 或 phase}-{议题}`，例 `warden-v0.1-001-foundation-check`
