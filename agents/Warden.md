...
```
读取 `.holdpoint/project/specs/{Spec-ID}/story.md`
├─ 长度 ≥ 50 字？     （避免空壳 story）
...
1. **运行 `hp warden foundation-check`** → 自动化检查 F1-F11，获取通过/拒绝结果
   ...
   - 输出 [通过] → 进入步骤 2
2. **读取 `.holdpoint/project/specs/{Spec-ID}/story.md`** → 按上面三条做内容合理性检查
...
**路径**：`.holdpoint/raw/{yy-mm-dd}/{session-id}.md`，`session-id = {agent}-{spec-id 或 phase}-{议题}`，例 `warden-v0.1-001-foundation-check`
